...
from settings import TARGET_IP, TARGET_MAC, WIFI_SSID, WIFI_PWD
...
pc_ip_address = TARGET_IP
pc_mac_address = TARGET_MAC
...
btn = Button(41,
             lambda: wifi_session.wol(pc_mac_address),
             set_exit_main)
led = Led(35)

while not exit_main:
    if not wifi_session.isconnected:
        print(wifi_session.my_ip)
        led.set_color(0, 200, 200)
        wifi_session.connect()
        
    t,r = ping(pc_ip_address, 1, timeout=1000)
    if t == r == 1:
    ...
    for i in range(15):
        if exit_main:
            break
        sleep(1)
        if time.time() - wifi_session.last_wol_time < 5:
            break

led.set_color(200, 0, 100)
